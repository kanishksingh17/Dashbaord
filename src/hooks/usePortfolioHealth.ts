...
export function usePortfolioHealth(): UsePortfolioHealthReturn {
  ...
  const fetchHealth = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch portfolio health");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const recomputeHealth = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to recompute portfolio health");
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to recompute portfolio health");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return { health, loading, error, refresh, recompute };
}
